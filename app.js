...
DATE_FORMAT(c.horaInicio, '%Y/%m/%d') as fechaCita, cli.idCliente, 
 ...
 AND c.idCentro = (SELECT idCentro FROM cita WHERE idCita = ?) ORDER BY fechaCita DESC`,[req.body.idCita,req.body.idCita]), 
   ...
   db(`SELECT cli.nombre as nombreCliente, cli.idCliente, cli.email, cli.idFoto, 
    (SELECT COUNT(idCita) FROM cita WHERE estado = 3 AND idCliente = cli.idCliente AND idCentro = ?) as completadas,
    (SELECT idCentro FROM cita WHERE idCita = ?) as idCentro,
(SELECT COUNT(idCita) FROM cita WHERE (estado = 1 OR estado = 0) AND idCliente = cli.idCliente AND idCentro = ?) as programadas, (SELECT COUNT(idCita) FROM cita WHERE estado = 4 AND idCliente = cli.idCliente AND idCentro = ?) as canceladas
FROM cliente as cli WHERE cli.idCliente = (SELECT idCliente FROM cita WHERE idCita = ?)
`,[req.body.idCentro,req.body.idCita,req.body.idCentro,req.body.idCita])])
      .then((data) => {
        if (!data) res.send().status(500);
