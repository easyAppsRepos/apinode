...
(SELECT uc.estado FROM usuario_consola as uc WHERE uc.email = c.email) as estado,
 ...
 (SELECT COUNT(r.idCita) FROM cita as r WHERE r.horaFinalEsperado > CURRENT_TIMESTAMP AND r.estado IN (1,2,5) AND r.idCentro = c.idCentro) as activos,
  ...
  FROM centro as c WHERE c.idCentro IN (SELECT f.idCentro FROM usuario_consola_centro as f WHERE f.idUsuarioConsola = ?)`,[req.body.idUsuarioConsola]),
     ...
     Promise.all([db(` SELECT c.*,
       TRUNCATE((SELECT SUM(r.comision),2) FROM cita as r 
 ...
 (SELECT COUNT(r.idCita) FROM cita as r WHERE r.estado = 4 AND r.idCentro = c.idCentro) as canceladas,
  TRUNCATE((SELECT SUM(r.precioEsperado)*(SELECT valor/100 FROM parametros 
  WHERE idParametro = 1) FROM cita as r WHERE r.estado = 4 
  AND r.idCentro = c.idCentro),2) as comisionCanceladas,
  TRUNCATE((SELECT SUM(r.precioEsperado) 
  FROM cita as r WHERE r.estado = 4 
  AND r.idCentro = c.idCentro),2) as canceladasT,
 (SELECT COUNT(r.idCita) FROM cita as r WHERE r.estado IN (1,5)  AND r.idCentro = c.idCentro) as porconfirmar,
  TRUNCATE((SELECT SUM(r.precioEsperado) 
  FROM cita as r WHERE r.estado 
  IN (1,5) AND r.idCentro = c.idCentro),2) as porconfirmarT,
  TRUNCATE((SELECT SUM(r.precioEsperado)*(SELECT valor/100 FROM parametros 
  WHERE idParametro = 1) 
  FROM cita as r 
  WHERE r.estado IN (1,5)  AND r.idCentro = c.idCentro),2) as comisionPorConfirmar,
   (SELECT COUNT(r.idCita) FROM cita as r WHERE r.estado = 5 AND r.idCentro = c.idCentro) as econfirmar,
     TRUNCATE((SELECT SUM(r.precioEsperado) 
     FROM cita as r 
     WHERE r.estado = 5 
     AND r.idCentro = c.idCentro),2) as econfirmarT,
  TRUNCATE((SELECT SUM(r.precioEsperado)*(SELECT valor/100 FROM parametros 
  WHERE idParametro = 1) 
  FROM cita as r WHERE r.estado = 5 
  AND r.idCentro = c.idCentro),2) as comisionEConfirmar,
   (SELECT COUNT(r.idCita) FROM cita as r WHERE r.estado = 2 AND r.idCentro = c.idCentro) as confirmadas,
   TRUNCATE((SELECT SUM(r.precioEsperado) 
   FROM cita as r WHERE r.estado = 2 
   AND r.idCentro = c.idCentro),2) as confirmadasT,
  TRUNCATE((SELECT SUM(r.precioEsperado)*(SELECT valor/100 
  FROM parametros WHERE idParametro = 1) 
  FROM cita as r WHERE r.estado = 2 
  AND r.idCentro = c.idCentro),2) as comisionConfirmadas,
   (SELECT COUNT(r.idCita) FROM cita as r WHERE r.estado = 3 AND r.idCentro = c.idCentro) as completadas,
    TRUNCATE((SELECT SUM(r.precioEsperado) FROM cita as r 
    WHERE r.estado = 3 AND r.idCentro = c.idCentro),2) as completadasT,
  ...
  TRUNCATE((SELECT SUM(r.precioEsperado)*(SELECT valor/100 FROM parametros 
  WHERE idParametro = 1) 
  FROM cita as r WHERE r.estado = 3 AND r.idCentro = c.idCentro),2) as comisionCompletadas 
