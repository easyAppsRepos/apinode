  ...
  expressApp.post('/verificarPremioUs', (req, res) => {
   Promise.all([db(`INSERT INTO premiosPuntos (puntos, idCliente) SELECT 15, ? WHERE 
      ...
      u.fbId, u.idFoto, u.estado, COUNT(c.idCita) as completadas,
       ((SELECT SUM(pp.puntos) FROM premiosPuntos as pp WHERE pp.idCliente = u.idCliente AND pp.estado = 1)+(SELECT SUM(f.exp) FROM cita as f WHERE f.idCliente = u.idCliente AND f.estado = 3)) as exp,
