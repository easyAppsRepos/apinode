...
*/
    ...
    let mailOptions = {
        from: 'yourBeautyMessageCenter@gmail.com', // sender address
        to: email, // list of receivers
        replyTo:'soporte@yourbeauty.com.pa',
...
Promise.all([
    db(`UPDATE cita set  estado=3,
      ...
      comision=1 WHERE idCita = ?`,[idCita,idCita]), 
...
// ORDER BY FIELD(r.estado, 5, 1, 2,3,4), r.horaInicio DESC
  expressApp.post('/reservasUser', (req, res) => {
    db(`SELECT c.nombre as nombreCentro, c.idFoto, r.idCita, r.idCentro, r.horaInicio, ee.nombre as nombreEmpleado,
      ...
      WHERE c.idCentro = r.idCentro AND r.idCliente = ? ORDER BY r.horaInicio ASC`,[req.body.idCliente])
      .then((data) => {
        if (!data) res.send().status(500);
        return res.send(data);
      }).catch(err => res.send(err).status(500));
    ...
    expressApp.post('/getHorasDispo', (req, res) => {
      ...
      var controlIncremento = moment({year:fecha.split('-')[0],month:(parseInt(fecha.split('-')[1])-1),
day:fecha.split('-')[2], hours: req.body.horaAbrir.split(':')[0], minutes: req.body.horaAbrir.split(':')[1]}).add(15,'m');
...
DATE_FORMAT(c.horaInicio, '%Y/%m/%d') as fechaCita, cli.idCliente, 
 c.precioEsperado, c.idPaquete, c.comentarioCita, c.idCuponCliente,c.idCita as idCita, cli.nombre as nombreCliente,
...
*/
  ...
  expressApp.post('/agregarOpinion', (req, res) => {
    ...
    expressApp.post('/getActualizacionMes', (req, res) => {
...
db(`UPDATE servicio_cita set estado=? 
     ...
     Promise.all([db(`UPDATE cita set estado=?, adminAction = 1 WHERE ? <> 4 AND idCita = ? AND ? = 
...
(SELECT uc.plan FROM usuario_consola as uc WHERE uc.email = c.email) as tipoContrato,
...
(SELECT uc.estado FROM usuario_consola as uc WHERE uc.email = c.email) as estado,
 ...
 (SELECT COUNT(r.idCita) FROM cita as r WHERE r.estado = 7 AND r.idCliente <> 0 AND r.idCentro = c.idCentro) as canceladas,
  (SELECT COUNT(r.idCita) FROM cita as r WHERE  r.idCliente = 0 AND r.idCentro = c.idCentro) as manuales,
...
`,[req.body.lat, req.body.lon, req.body.lat])
   ...
   * sin( radians(c.latitud)))) AS distance 
      FROM servicio as s, centro as c LEFT JOIN evaluacionCentro as ec ON ec.idCentro = c.idCentro
      WHERE c.idCentro = s.idCentro 
      AND s.idSubcategoria IN (`+req.body.idSubcategoria.toString()+`) AND c.estado = 1 
      AND s.estado = 1   
      GROUP BY c.idCentro HAVING distance < 35`,[req.body.lat, req.body.lon, req.body.lat])
      .then((data) => {
        if (!data) res.send().status(500);
...
`,[req.body.idCupon])
  ...
  expressApp.post('/getOpiniones', (req, res) => {
    db(`SELECT ec.idEvaluacionCentro, ec.servicio, 
      ...
      ec.comentario, ec.respuestaCentro,ec.puntuacion, ec.fechaCreacion, ci.idCita, c.nombre, c.idFoto, ci.horaFinalEsperado, ci.horaInicio, ci.precioEsperado
 FROM evaluacionCentro as ec, centro as c, cita as ci 
 WHERE ec.idCentro = c.idCentro AND ec.idCita = ci.idCita AND  ci.idCliente = ? ORDER BY ci.horaInicio ASC`,[req.body.idCliente])
      .then((data) => {
        if (!data) res.send().status(500);
...
db(`SELECT nombre FROM centro WHERE idCentro = ?`,[idCentro])])
 ...
 var clave = makeid();
  ...
  expressApp.post('/addServicioNC', function(req, res) {
    ...
    expressApp.post('/loginNC2', (req, res) => {
...
const serverHttpss = https.createServer(https_options,expressApp);

serverHttpss.listen(8282, () => console.log(`Running on localhost:8282`));
