...
ORDER BY c.porcentajeDescuento DESC LIMIT 1`,[req.body.idCentro,req.body.idCliente])])
  ...
  expressApp.post('/getCuponesAll', (req, res) => {
    ...
    db(`SELECT s.idServicio, s.nombre, s.duracion, s.precio, s.idCategoria, s.descripcion, c.nombre as nombreCategoria, cs.nombre as nombreSubcategoria   
      FROM servicio as s, categoria as c, subcategoria as cs,  (SELECT precioOferta FROM control_oferta as co WHERE co.idServicio = s.idServicio AND co.fechaCaducidad>CURRENT_TIMESTAMP  LIMIT 1) as oferta   
